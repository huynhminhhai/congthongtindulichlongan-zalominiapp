...
const AvatarUploaderSingle: React.FC<AvatarUploaderSingleProps> = ({ label, error, field, required = false }) => {
          ...
          <div onClick={handleChooseImage} className="cursor-pointer">
            {/* {field.value ? (
              <div className="relative w-36 h-36">
                <img src={field.value} alt="Uploaded" className="w-full h-full object-cover rounded-full" />
                <button
...
const FormAvatarUploaderSingle: React.FC<FormAvatarUploaderSingleProps> = ({
  ...
  return (
    <Controller
      ...
      control={control}
      render={({ field }) => (
        <AvatarUploaderSingle label={label} name={name} required={required} field={field} error={error} />
      )}
    />
  );
};
