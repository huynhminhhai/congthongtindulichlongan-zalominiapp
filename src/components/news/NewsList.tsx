    ...
    <Box>
      <Box>
        <FilterBar
          showAddButton={false}
          searchComponent={
            <Input.Search placeholder={tCommon('searching')} value={search} onChange={e => setSearch(e.target.value)} />
          }
        >
          <div className="col-span-12">
            <Select placeholder="Văn hóa" closeOnSelect>
      ...
      <Box>
        {isLoading ? (
          ...
          <div className="grid grid-cols-1">
            {listData[0] && (
              <Box mb={4}>
                <div onClick={() => navigate(`/news-detail/${listData[0].id}`)}>
                  <img
                  ...
                  <Box px={4}>
                    <div className="flex items-center justify-between mt-3 mb-2 pb-2 border-b-[1px] border-[#355933]">
                      ...
                      <div className="text-[12px] leading-[1] font-medium">
                        {listData[0].publishedDate || '12/12/2025'}
                      </div>
                    </div>
            ...
            <Box px={4}>
              {listData.slice(1).map(item => (
                <Box key={item.id} onClick={() => navigate(`/news-detail/${item.id}`)} mb={4}>
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <h3 className="text-[15px] leading-[20px] font-semibold whitespace-normal mt-1 line-clamp-2">
                        {item.id} - {item.title}
                      </h3>
      ...
      <div ref={loaderRef} className="px-4">
        ...
        {listData.length > 0 && !hasNextPage && <p className="text-center pt-4">{tCommon('all-displayed')}</p>}
      </div>
    </Box>
  );
};
