            ...
            <Box>
                <div className="grid grid-cols-1">
                    {(listData.length === 0 && !isFetchingNextPage && !isLoading) ? (
                        <Box px={4}>
                            <EmptyData title="Hiện chưa có sự kiện nào!" desc="Khi có sự kiện, bạn có thể thao tác ngay tại đây. Vui lòng quay lại sau!" />
                        </Box>
                    ) : (
                        <>
                                    ...
                                    <div onClick={() => navigate(`/events-detail/?id=${listData[0].id}`)}>
                                        ...
                                        <Box px={4}>
                                            <div className="flex items-center justify-between mt-3 mb-2 pb-2 border-b-[1px] border-[#355933]">
                                                <div className="text-[13px] leading-[1] font-semibold">{listData[0].category || 'Sự kiện'}</div>
