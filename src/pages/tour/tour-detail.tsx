...
import { tourData } from 'components/PostComponent/tour/TourSection';
            ...
            <Box mb={9}>
              <Box>
                <ul className="flex flex-col gap-3 font-medium">
                  ...
                  <li>MÃ TOUR: HUFC7P</li>
                  <li>Phương tiện di chuyển: Ô tô</li>
                  <li>Thời gian: 1 Ngày</li>
                  <li>Nơi khởi hành: TP. Hồ Chí Minh</li>
                  <li>Ngày khởi hành: Chủ Nhật</li>
                </ul>
              </Box>
            </Box>
              ...
              {tourData.map((item, index) => (
                <Box flex className="gap-3" key={index} onClick={() => navigate('/tour-detail')}>
                  <ItemDetailCard
                    imgUrl={item.imgUrl}
                    title={item.title}
                    desc={`${item.place} địa điểm - ${item.date} ngày`}
                  />
                </Box>
              ))}
            </div>
