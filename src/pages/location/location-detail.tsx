...
const LocationDetailPage = () => {
  ...
  return (
    <Page className="relative flex-1 flex flex-col bg-white">
      <Box>
        <HeaderSub title={tPage('destination-detail')} />
        <Box>
          <Box mb={2}>
        ...
        <Box px={4} pb={4}>
          <TitleSection title={tPage('destination-others')} mB={2} handleClick={() => navigate('/location')} />
          <Box pt={4}>
            <div className="grid grid-cols-1 gap-3">
              {DIA_DIEM_DU_LICH_DATA.map((item, index) => (
                <Box key={index} onClick={() => navigate(`/location/${item.id}`)}>
