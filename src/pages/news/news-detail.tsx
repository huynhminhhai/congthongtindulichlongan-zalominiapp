...
import { NewsOthers } from 'components/PostComponent/news';
...
const NewsDetailPage: React.FC = () => {
  ...
  return (
    <Page className="relative flex-1 flex flex-col bg-white">
      <Box>
        <HeaderSub title={tPage('news-detail')} />
        {isLoading ? (
          ...
          <Box px={4} pb={10}>
            <EmptyData title="Bài viết không tồn tại!" desc="Không thể tìm thấy bài viết bạn yêu cầu" />
          </Box>
        )}
        <Box pt={4} px={4}>
          <TitleSection title={tPage('news-others')} mB={2} handleClick={() => navigate('/news')} />
