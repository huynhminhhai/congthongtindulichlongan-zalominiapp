...
export const InforItemMain = ({
  ...
  return (
    <div className="flex items-center justify-between gap-6 py-4 resident-item">
      <div className="text-[14px] text-[#767a7f] font-normal whitespace-nowrap">
        {label}
      </div>
          ...
          {detailData && (
            <Box>
              <Box>
                <div className="bg-[#731611] relative flex flex-col items-center justify-center py-[30px] overflow-hidden">
                  ...
                  <img
                    src={images.shape2}
                    alt="shape"
                    className="absolute top-0 left-0 w-fit h-auto opacity-[0.1] z-0"
                  />
                  <Avatar
                    size={120}
                    src={
                  ...
                  />
                  <div className="relative z-20 uppercase text-[18px] font-bold mt-3 text-white">
                    {detailData.fullname}
                  </div>
                </div>
              </Box>
              <Box p={4}>
                <InforItemMain
                  label="Chủ hộ"
                  value={detailData.parentName || 'Là chủ hộ'}
                />
                <InforItemMain
                  label="Số định danh cá nhân"
                  value={detailData.numberCard}
                />
                <InforItemMain
                  label="Giới tính"
                  value={getLabelOptions(detailData.gender, gender) as string}
                />
                <InforItemMain label="Ngày sinh" value={detailData.birthDate} />
                <InforItemMain
                  label="Dân tộc"
                  value={
                    getLabelOptions(detailData.nation, ethnicOptions) as string
                  }
                />
                <InforItemMain
                  label="Tôn giáo"
                ...
                <InforItemMain
                  ...
                  value={detailData.culturalFamilyStatus ? 'Đạt' : 'Không đạt'}
                />
              </Box>
            </Box>
          )}
