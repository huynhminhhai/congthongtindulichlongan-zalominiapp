...
const commentsApiRequest = {
    getCommentsList: async (param: { paramName: string; itemId: number; page: number; pageSize: number }) => {
        return await http.get<any[]>(`/comments?${param.paramName}=${param.itemId}&_page=${param.page}&_limit=${param.pageSize}`);
    },
...
export const useGetCommentsList = (paramName: string, itemId: number, pageSize: number) => {
